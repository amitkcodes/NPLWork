...
# Dictionary to track the last alert time for each server (used for rate-limiting)
last_alert_time = {}
...
def get_offset_and_delay_from_ntp(server, alert_emails):
    ...
    try:
        ...
        # Return offset, delay, and data
        return offset_in_sec, delay_in_sec, data
    except ntplib.NTPException as e:
        # Handle NTP-specific errors
    ...
    if valid_offsets:
        ...
        for _, (offset, delay, data) in results:
            if offset != 0:
                ...
                # Alert if deviation exceeds 1 ms (0.001 seconds)
                if abs(diff) > 0.01:
                    alert_message = (
...
def run_ntp_monitoring(ntp_servers, alert_emails):
    try:
        while True:
            ...
            print("Waiting for 30 minutes before collecting data again...")
            time.sleep(1800)
...
if __name__ == '__main__':
    ...
    ntp_servers = [
        '157.20.66.8', 'ntp.doca.gov.in', 'samay2.nic.in',
    ...
    alert_emails = ['amitnplindia21@gmail.com']  # Second email commented out
